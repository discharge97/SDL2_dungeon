...
void mainMenu(SDL_Renderer *pRenderer, SDL_Texture *pTexture);

Mix_Chunk *musicBackground;
Mix_Chunk *shoot;
Mix_Chunk *step;
short pauseGame = 0;
short menu = 1;
...
int main(int argc, char** argv) {
    ...
    musicBackground = Mix_LoadWAV(MUSIC_RES);
    shoot = Mix_LoadWAV(SHOOT_RES);
    step = Mix_LoadWAV(STEP_RES);
    ...
    surface = IMG_Load(MENUBG_RES);
    texMenu = SDL_CreateTextureFromSurface(renderer, surface);
    SDL_FreeSurface(surface);
	...
	int musicChannel = Mix_PlayChannel(-1, musicBackground, -1);
	Mix_Volume(musicChannel, 20);

	while (running && menu >= 0) {

		while (SDL_PollEvent(&event)) {
			Input(&state, &event, &running);
		}

        // update menu after choosing start game to false
		if(menu == False) {
            if (pauseGame == 0) {
                ...
                draw_text(renderer, font, "GAME PAUSED",
...
void Render(state_t* state, SDL_Renderer* renderer, SDL_Texture* tex, TTF_Font* font) {
    ...
    #define plvlxy(ox, oy) state->level.pickups[((y + yoff + (oy)) * LVL_W) + (x + xoff + (ox))]
	...
	for (y = 0; y < SCR_H; ++y) {
		for (x = 0; x < SCR_W; ++x) {
			...
			if (state->ren_mode == REN_BLOCKS || state->ren_mode == REN_ALL) {
                ...
                switch (plvlxy(0, 0)) {
                    case P_POTION:
                        load_sprite(SPR_POTION, (spr_rect *) &src);
                        break;
                    case P_POWERUP:
                        load_sprite(SPR_POWERUP, (spr_rect *) &src);
                        break;
                }
                SDL_RenderCopy(renderer, tex, &src, &dest);
	...
	snprintf(text_buf, 127, "Enemies: %3s | Light: %4s | Render: %8s", state->spawn_enemies ? "ON" : "OFF",
